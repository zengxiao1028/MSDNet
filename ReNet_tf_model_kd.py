...
num_classes = 10
height_width = 224
num_student = 512  # number of element of student-teacher KD
# checkpoint initialization
checkpoint_dir_load = './models_tf/'
checkpoint_dir = './models_tf_kd/'
model_tag = 'VGG_10p'
...
class VGG:
    ...
    def train(self, train_generator, validation_generator, epochs, train_steps, validation_steps):
        ...
        if load(saver, self.sess, checkpoint_dir_load):
        ...
        for j in range(epochs):
            ...
            for i in range(train_steps):
                ...
                _, l, k, a = self.sess.run([self.train_op, self.loss_entropy, self.kd_dist, self.accuracy], feed_dict={
                    self.x: batch[0],
                    self.y: y,
                    self.vec512: batch[1][:, 1:]
                })
                loss += l
                acc += a
                kd += k
                print("\rEpoch = %d/%d; Progress = %d/%d; loss_entropy = %.3f; kd_dist = %.3f; acc = %.4f" % (
                    j + 1, epochs, (i + 1), train_steps, l, k, a), end='')

            print("\rEpoch = %d/%d; loss_entropy = %.3f; kd_dist = %.3f; acc = %.4f" % (
                j + 1, epochs, loss / train_steps, kd / train_steps, acc / train_steps))
    ...
    def validate(self, validation_generator, validation_steps):
        # validate
        # print('..', load(tf.train.Saver(max_to_keep=1), self.sess, checkpoint_dir_load))
        val_loss = 0
        val_acc = 0
        val_kd = 0
        for i in range(validation_steps):
            ...
            val_acc += a
            val_kd += k
        print("val_loss_entropy = %.3f; val_kd = %.3f; val_acc = %.4f" % (
            val_loss / validation_steps, val_kd / validation_steps, val_acc / validation_steps))
        return val_acc
...
def VGG_preload(x, conv_setting, weights, recover_setting, freeze=True):
    ...
    with tf.name_scope('fc1'):
        ...
        vector_512 = tf.matmul(layer, W_fc1) + b_fc1
        h_fc1 = tf.nn.relu(vector_512)
        # dropout here
